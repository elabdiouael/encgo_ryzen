...
import { Home, Info, HelpCircle, Database, Handshake, Mail, Users, Menu, X } from 'lucide-react';
...
export default function Navbar() {
  ...
  // K-n-chekkou wach 7na f Mobile awla PC bach n-bloquiw l-Hover
  useEffect(() => {
    ...
    <div className={styles.fixedCenterWrapper}>
      ...
      >
        ...
        <button className={styles.mobileToggle} onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
          {mobileMenuOpen ? <X size={24} color="#00d2ff" /> : <Menu size={24} color="#00d2ff" />}
        </button>
      ...
      {mobileMenuOpen && (
        <div className={styles.mobileNav}>
          {links.map((link) => (
            ...
            >
              {link.icon && <span className={styles.mobIcon}>{link.icon}</span>}
              {link.name}
            </Link>
          ))}
